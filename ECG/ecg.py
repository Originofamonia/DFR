...
class ECG:
    def __init__(self):
        self.n = 50  # number of nodes, n-1 virtual nodes
        ...
        val = mat['val'].reshape(-1)
        self.input = val[0: self.k]
        self.target = val[self.k: 2 * self.k]
    ...
    def draw_charts(self):
        ...
        plt.grid()
        plt.title('Train: Reservoir Output vs Actual Output for ECG-input')
        ...
        plt.xlabel('o: Reservoir, x: Actual')
        plt.grid()
        plt.title('Test: Reservoir Output vs Actual Output for ECG-input')
        # Below two lines are used to save the results.
        # mat = np.column_stack((x2, self.output_test, self.target2))
        # np.savetxt('ecg_result.csv', mat, delimiter=', ')
...
def main():
    ...
    mse, nmse = e1.training_error()
    print('mse: ', mse, ', nmse: ', nmse)
    mse_test, nmse_test = e1.test()
